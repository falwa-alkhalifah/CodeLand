...
if (!$quizID) {
    header("Location: educator_homepage.php?error=missingQuizId");
    exit;
}
...
if (!$quizRow) {
    header("Location: educator_homepage.php?error=quizNotFound");
    exit;
}

// مملوك لنفس المعلّم؟
if ((int)$quizRow['educatorID'] !== $educatorId) {
    header("Location: educator_homepage.php?error=notOwner");
    exit;
}
...
$st = $conn->prepare("SELECT photoFileName FROM user WHERE id=?");
$st->bind_param("i", $educatorId);
$st->execute();
$meData = $st->get_result()->fetch_assoc();
$st->close();

$stored = $meData['photoFileName'] ?? '';
...
<div class="container">
    ...
    <!-- ✅ REMOVED the empty container here -->
